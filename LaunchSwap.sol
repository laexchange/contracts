...
contract LaunchSwapPair is IUniswapV2Pair, UniswapV2ERC20, Initializable {
    ...
    function __LaunchSwapPair_init(address _token0, address _token1, uint _vReserve0, uint _vReserve1) external initializer {
        ...
        token0 = _token0;
        token1 = _token1;
        require(_vReserve0 <= uint112(-1) && _vReserve1 <= uint112(-1), "UniswapV2: OVERFLOW");
    ...
    function _update(uint balance0, uint balance1, uint vr0, uint vr1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0.add(vr0) <= uint112(-1) && balance1.add(vr1) <= uint112(-1), "UniswapV2: OVERFLOW");
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out <= uint(_reserve0).sub(vReserve0) && amount1Out <= uint(_reserve1).sub(vReserve1), "UniswapV2: INSUFFICIENT_LIQUIDITY");
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(uint(_reserve1).mul(1e8)), "UniswapV2: K");
        }
...
contract LaunchSwapFactory is IUniswapV2Factory {
    ...
    function createPairVR(address token, address currency, uint vReserve) public returns (address pair) {
        require(vReserve == 0 || Math.max(IERC20(token).balanceOf(msg.sender), IERC20(token).balanceOf(tx.origin)).mul(1 ether) >= IERC20(token).totalSupply().mul(holdRateVR), "Only more than holdRateVR token holder can create pair with virtual reserve");
        require(token != currency, "UniswapV2: IDENTICAL_ADDRESSES");
    ...
    function createPairVRR(address token, address currency, uint vReserve, uint _feeRate, uint _taxRate) public returns (address pair) {
        require(_feeRate <= 1 ether && _taxRate <= 1 ether, "feeRate and taxRate MUST NOT great than 100%");
...
contract LaunchSwapRouter02 is IUniswapV2Router01, Initializable {      //IUniswapV2Router02, 
    ...
    function checkbuyback(address buyback_) internal view returns (address) {
        buyback;
        require(buyback_ == address(0), "buyback_ MUST be NULL address");
        return buyback_;
    }

    function createToken(string memory name_, string memory symbol_, uint8 decimals_, uint totalSupply_, address to) public returns (address token) {
        token = NewLib.newUniswapV2ERC20(name_, symbol_, decimals_, totalSupply_, to);
        allTokens.push(token);
    }

    function createTokenAndLiquidity0ETH(string calldata name, string calldata symbol, address buyback_) external payable returns (address token, address pair, uint amount) {
        token = createToken(name, symbol, 18, 1_000_000_000e18, address(this));
        (,,, pair) = PoolLib.createLiquidity0ETH(factory, token, WETH, 1_000_000_000e18, 5 ether);
    ...
    function createLiquidity0ETH(
        ...
        (amount, value, liquidity, pair) = PoolLib.createLiquidity0ETH(factory, token, WETH, amountDesired, valueDesired);
    ...
    function createLiquidity0(
        ...
        return PoolLib.createLiquidity0(factory, token, currency, amountDesired, valueDesired);
    }
    ...
    function addLiquidity(
        ...
        address to,
        uint deadline
    ) external returns (uint amountA, uint amountB, uint liquidity) {                              // virtual override 
        return PoolLib.addLiquidity(factory, tokenA, tokenB, amountADesired, amountBDesired, amountAMin, amountBMin, to, deadline);
    }
    function addLiquidityETH(
        address token,
        uint amountTokenDesired,
        ...
        address to,
        uint deadline
    ) external payable returns (uint amountToken, uint amountETH, uint liquidity) {                // virtual override 
        return PoolLib.addLiquidityETH(
            factory, 
            token,
            WETH,
            amountTokenDesired,
    ...
    function removeLiquidity(
        ...
        address to,
        uint deadline
    ) public returns (uint amountA, uint amountB) {        // virtual override 
        return PoolLib.removeLiquidity(factory, tokenA, tokenB, liquidity, amountAMin, amountBMin, to, deadline);
    }
    function removeLiquidityETH(
        address token,
        uint liquidity,
        ...
        return PoolLib.removeLiquidityETH(factory, token, WETH, liquidity, amountTokenMin, amountETHMin, to, deadline);
    }
    function removeLiquidityWithPermit(
        address tokenA,
        address tokenB,
        ...
        PoolLib.permitLiquidity(factory, tokenA, tokenB, liquidity, deadline, approveMax, v, r, s);
    ...
    function removeLiquidityETHWithPermit(
        ...
        PoolLib.permitLiquidity(factory, token, WETH, liquidity, deadline, approveMax, v, r, s);
    ...
    // Reserved storage space to allow for layout changes in the future.
    uint256[50-3] private ______gap;
}
...
contract LimitedERC20 is UniswapV2ERC20 {
    ...
    function modifyCurrencies(address[] calldata currencies, bool isAdd) external {
        require(LaunchSwapFactory(factory).isSetter(msg.sender), "FORBIDDEN");
    ...
    function buybackThenBurn(address currency, uint value, uint minAmount, uint deadline) external returns (uint amount) {
        require(LaunchSwapFactory(factory).isSetter(msg.sender), "FORBIDDEN");
        address[] memory path = new address[](2);
        path[0] = currency;
        path[1] = address(this);
...
contract LaunchPool {
    ...
    function setTime_(uint duration_, uint begin) external {
    ...
    function withdraw(uint value) public invest(msg.sender) returns (uint) {
    ...
    event Withdrawn(address indexed account, uint value);
    ...
    function exit() external returns (uint amt, uint vol) {
        amt = withdraw(uint(-1));
        vol = claim();
    }
    ...
    function _invest(address account) internal {
        ...
        if(value == 0) {
            return;
        } else if(value < totalCurrencyLast) {
    ...
    function quotaDelta() public view returns(uint quota, uint value, address[] memory path) {
        ...
        if(lasttime == 0 || lasttime >= now || totalCurrencyLast == 0)    // not init, not begin, finished    
            return (0, 0, path);
        quota = IERC20(quotaToken).balanceOf(address(this)).mul(now.sub(lasttime)).div(duration);
    ...
    function symbol() external view returns (string memory) {
        return string(abi.encodePacked("la", IERC20(currency).symbol(), ">", IERC20(underlying).symbol()));
    }
...
contract QuotaFarmingRouter is LaunchSwapRouter02, IUniswapV2Callee {
    ...
    function __QuotaFarmingRouter_init(address _factory, address _WETH, uint begin_) public initializer {
        __LaunchSwapRouter02_init(_factory, _WETH);
        __QuotaFarmingRouter_init_unchained(begin_);
    }

    function __QuotaFarmingRouter_init_unchained(uint begin_) public onlySetter {
        duration = 20 days;
        creatorRatio = 0.40 ether;
        buybackRatio = 0.40 ether;
        (LA,,,) = createQuotaPool("LA.exchange", "LA", begin_);
    }
    ...
    function createLimitedToken(string memory name_, string memory symbol_, uint8 decimals_, uint totalSupply_, address to, address[] memory currencies) public returns (address limitedToken, address quotaToken) {
    ...
    function createQuotaPool(string memory name, string memory symbol, uint begin_) public returns (address limitedToken, address quotaToken, address pair, address pool) {
        address[] memory currencies = new address[](1);
        currencies[0] = WETH;
        (limitedToken, quotaToken) = createLimitedToken(name, symbol, 18, 1_000_000_000e18, address(this), currencies);
        ...
        pool = PoolLib.newLaunchPool(WETH, limitedToken, quotaToken, duration, begin_);
    ...
    function checkbuyback(address buyback_) internal view returns (address) {
        require(quotaTokenOf[buyback_] != address(0), "buyback_ MUST be LimitedERC20");
        return buyback_;
    }
    ...
    function quotaDelta(address limitedToken, address taxToken) public view returns (uint amt) {
        ...
        if(lep[limitedToken][taxToken] == 3) {                                                              // power
            //uint y = period.mul(1 ether).div(lastUpdateTime.add(quotasDuration).sub(begin));
            //uint amt1 = amt.mul(1 ether).div(y);
            //uint amt2 = amt1.mul(period).div(now.add(quotasDuration).sub(begin));
    ...
    function earned(address account, address limitedToken, address taxToken) public view returns (uint) {
    ...
    function uniswapV2Call(address token, uint taxToken, uint amountTax, bytes calldata data) external {
        data;
        require(msg.sender == UniswapV2Library.pairFor(factory, token, address(taxToken)), "msg.sender MUST be LaunchSwapPair");
