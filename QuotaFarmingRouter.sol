...
contract LaunchPool {
    ...
    function exit() external returns (uint amt, uint tax, uint vol) {
        (amt, tax) = withdraw(totalCurrencyLast);
        vol = claim();
    }
...
contract LaunchPool2 is ConstSepolia {
    ...
    function exit() external returns (uint[] memory amts, uint[] memory taxs, uint vol) {
        ...
        for(uint i=0; i<N; i++) {
            ...
            if(balanceOf[msg.sender][currency] > 0)
                (amts[i], taxs[i]) = withdraw(currency, totalCurrencyLast[currency]);
        }
        vol = claim();
    }
    ...
    function _invest(address account, address currency) internal {
        ...
        if(value == 0) {
            return;
        //} else if(value < totalCurrencyLast[currency]) {
        //    value = router.swapTokensForExactTokens(quota, totalCurrencyLast[currency], path0Und, address(this), now)[0];
        } else {
            if(currency != currencies[0]) {
                require(currencies[0] == WETH && (currency == USDT || currency == USDC || currency == USDX), "Not expected currencies");
                uint amount = value.mul(currency == USDX ? 1e8 : 1e18/1e6*1e8).div(uint(AggregatorInterface(oracleETH).latestAnswer()));
                value = uniRouter.swapExactTokensForTokens(value, 0, pathCur0, address(this), now)[1];
                require(value.mul(1e18) >= amount.mul(uint(1e18).sub0(slippage)), "Slippage");
            }
